...
if (length(args) == 0) {
  cat("Usage: Rscript trim_existing_models_aggressive.r <input_dir> [backup_dir]\n")
  cat("  input_dir: Directory containing .rds model files to process\n")
  cat("  backup_dir: Optional directory to backup original files (default: <input_dir>_backup)\n")
  cat("\nExample:\n")
  cat("  Rscript trim_existing_models_aggressive.r /path/to/models\n")
  cat("  Rscript trim_existing_models_aggressive.r /path/to/models /path/to/backup\n")
  quit(status = 1)
}
...
analyze_object_sizes <- function(obj, name = "object", threshold_mb = 1, max_depth = 6, current_depth = 0) {
  ...
  if (size_mb >= threshold_mb) {
    ...
    if (is.list(obj) && length(obj) > 0 && !is.matrix(obj)) {
      for (i in seq_along(obj)) {
        element_name <- if (is.null(names(obj))) as.character(i) else names(obj)[i]
        if (is.null(element_name) || element_name == "") element_name <- as.character(i)
        
        child_name <- paste0(name, "$", element_name)
        child_results <- analyze_object_sizes(obj[[i]], child_name, threshold_mb, max_depth, current_depth + 1)
...
safe_remove_component <- function(obj, path_string) {
  ...
  if (length(path_components) > 1) {
    for (i in 1:(length(path_components) - 1)) {
      ...
      if (component %in% names(current_obj)) {
        ...
        return(list(obj = obj, removed = FALSE, size_mb = 0, reason = paste("Path not found:", paste(path_components[1:i], collapse = "$"))))
      }
    }
  }
  ...
  if (final_component %in% names(current_obj)) {
    size_mb <- as.numeric(object.size(current_obj[[final_component]])) / (1024^2)
    ...
    if (length(parent_path) == 0) {
      ...
      removal_expr <- paste0("obj_copy", paste0("$", parent_path, collapse = ""), "$", final_component, " <- NULL")
      eval(parse(text = removal_expr))
    }
    
    return(list(obj = obj_copy, removed = TRUE, size_mb = size_mb, reason = "Success"))
  } else {
    return(list(obj = obj, removed = FALSE, size_mb = 0, reason = paste("Component not found:", final_component)))
  }
}
...
aggressive_trim_model <- function(model_obj, filepath) {
  ...
  cat(sprintf("    Analyzing object structure (%.1f MB total)...\n", original_size))
  
  # Analyze all components > 0.5MB
  large_components <- analyze_object_sizes(model_obj, "obj", threshold_mb = 0.5, max_depth = 8)
  ...
  if (length(large_components) > 0) {
    ...
    for (i in sorted_indices) {
      ...
      cat(sprintf("      %s: %.1f MB (%s, len=%d, matrix=%s)\n", 
                  gsub("^obj\\$", "", comp_name), comp_info$size_mb, 
                  comp_info$type, comp_info$length, comp_info$is_matrix))
    }
  }
  ...
  for (target_path in aggressive_targets) {
    ...
    if (result$removed && result$size_mb > 0.1) {
      removed_components <- c(removed_components, 
  ...
  if (length(large_components) > 0) {
    for (comp_name in names(large_components)) {
      ...
      essential_patterns <- c("forest", "spec", "levels", "independent.variable.names", 
                            "class.values", "treetype", "splitrule")
      
      if (any(sapply(essential_patterns, function(p) grepl(p, comp_name, ignore.case = TRUE)))) {
        next
      }
      
      # If it's a large matrix or list that looks like training data, remove it
      if (comp_info$size_mb > 5 && 
          (comp_info$is_matrix || 
           grepl("template|mold|pred|inbag|importance|call|training|orig", comp_name, ignore.case = TRUE))) {
        ...
        if (result$removed) {
          ...
          total_removed_size <- total_removed_size + result$size_mb
          model_obj <- result$obj
          cat(sprintf("      ✓ Aggressively removed %s: %.1f MB\n", path_string, result$size_mb))
        }
      }
    }
  }
  ...
  reduction_pct <- if(original_size > 0) (reduction / original_size) * 100 else 0
  
  return(list(
    trimmed = length(removed_components) > 0,
    reason = if (length(removed_components) > 0) paste(removed_components, collapse = "; ") else "No removable components found",
...
for (i in seq_along(rds_files)) {
  ...
  tryCatch({
    ...
    if (trim_result$trimmed) {
      ...
      cat(sprintf("  ✓ HEAVILY TRIMMED: %.1f MB → %.1f MB (%.1f%% reduction)\n", 
                  trim_result$original_size_mb, trim_result$final_size_mb, trim_result$reduction_pct))
      cat(sprintf("    File size: %.1f MB → %.1f MB\n", original_file_size, new_file_size))
      cat(sprintf("    Removed %d components totaling %.1f MB\n", 
                  trim_result$removed_count, trim_result$total_removed_size))
      
      processed_files <- processed_files + 1
    } else {
      total_size_after <- total_size_after + original_file_size
      cat(sprintf("  - SKIPPED: %s (%.1f MB)\n", trim_result$reason, trim_result$original_size_mb))
...
cat("\nStorage savings:\n")
cat(sprintf("Total size before: %.1f MB (%.2f GB)\n", total_size_before, total_size_before/1024))
cat(sprintf("Total size after: %.1f MB (%.2f GB)\n", total_size_after, total_size_after/1024))
cat(sprintf("Total reduction: %.1f MB (%.2f GB, %.1f%%)\n", 
