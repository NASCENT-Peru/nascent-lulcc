...
lulcc.plotBoyceandROCcurves <- function(
  RF_specs,
  GLM_specs,
  RF_model_num,
  transition_name
  ...
  RF_model_folder_paths <- lapply(RF_specs, function(x) {
    paste0(
      "outputs/Fitted_models/Fitted_rf_models/",
      x,
      "/",
      data_period_name,
      "_rf_models"
  ...
  GLM_model_folder_paths <- lapply(GLM_specs, function(x) {
    paste0(
      "outputs/Fitted_models/Fitted_glm_models/",
      x,
      "/",
      data_period_name,
      "_glm_models"
  ...
  Fitted_model_folder_paths <- append(
    RF_model_folder_paths,
    GLM_model_folder_paths
  )

  # load fitted models
  All_models <- unlist(
    lapply(
      Fitted_model_folder_paths,
      function(model_folder, RF_model_num) {
        # list all files
        ...
        single_trans_models <- file_list[grep(
          transition_name,
          names(file_list)
        )]
        ...
        if (grepl("RF", model_folder, ignore.case = TRUE)) {
          single_trans_models <- single_trans_models[c(grep(
            RF_model_num,
            names(single_trans_models)
          ))]
        }
        ...
        names(Fitted_models) <- lapply(names(Fitted_models), function(x) {
          str_remove_all(
            x,
            paste(
              c(paste0("_rf-", RF_model_num, ".rds"), "_glm.rds"),
              collapse = "|"
            )
          )
        })
  ...
  names(AUC_Boyce_results) <- lapply(names(All_models), function(x) {
    str_replace_all(
      str_remove(x, paste0("\\.", transition_name)),
      c(
        unfiltered = "full",
        filtered = "reduced",
        GLM = "LR",
  ...
  # Boyce curve
  Boyce_results_df <- rbindlist(
    lapply(AUC_Boyce_results, function(x) {
  ...
  # add columns
  Boyce_results_df$model <- sapply(Boyce_results_df$model_tag, function(x) {
    str_split(str_split(x, "\\.")[[1]][1], "_")[[1]][1]
  })
  ...
  ROC_results$model <- sapply(ROC_results$model_tag, function(x) {
    str_split(str_split(x, "\\.")[[1]][1], "_")[[1]][1]
  })
  ROC_results$label <- paste0(
    ROC_results$model,
    " ",
    "(AUC =",
    ROC_results$AUC,
  ...
  LR_results <- ROC_results[
    ROC_results$model == "LR" & ROC_results$TruePositive > 0.5
  ]
  ...
  ROC_plot <- ggplot(alpha = 0.7) +
    geom_line(
      data = ROC_results,
      aes(x = FalsePositive, y = TruePositive, colour = model)
    ) +
    ...
    geom_text(
      data = ROC_results |>
        filter(FalsePositive == max(FalsePositive)) |>
        group_by(model),
      size = 4,
      show.legend = FALSE,
      aes(
  ...
  ggsave(
    plot = ROC_boyce_plot,
    filename = paste0(
      "outputs/Figures/Model_evaluation/Boyce_ROC_curves/",
      str_replace_all(transition_name, "\\.", "_"),
      "_boyce_ROC_curve"
    ),
    device = "tiff",
    dpi = 300,
